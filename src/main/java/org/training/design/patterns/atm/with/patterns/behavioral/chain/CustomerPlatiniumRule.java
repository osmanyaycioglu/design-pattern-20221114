...
public class CustomerPlatiniumRule extends AbstractCustomerRule {

    public CustomerPlatiniumRule(ICustomerRule next) {
        super(next);
    }
    ...
    void internalRun(Customer customer) {
        ...
        if (customer.accountStream()
                    .noneMatch(a -> a.getAccountType() == EAccountType.EURO)) {
            throw new IllegalArgumentException("Euro hesabı yok");
        }
    }
}
